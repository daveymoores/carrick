...
- `crates/carrick-match/` is the path-matching crate (`paths_match`, `is_param_segment`) — the single source of matching semantics for both sides of Carrick. The scanner depends on it natively; every release also compiles it to wasm (feature `wasm`, `wasm-bindgen` pinned exactly — bump the CLI pin in `.github/workflows/ci.yml` in lockstep) and attaches `carrick_match.js` / `carrick_match.d.ts` / `carrick_match_bg.wasm` / `carrick_match.sha256` as release assets via `wasm-artifact.yml` (also runnable by `workflow_dispatch` to backfill a tag). The companion cloud repo pins those assets by hash and runs the same matcher at query time, so changing matching semantics here propagates on the next release — never fork matching logic elsewhere.
