...
impl MongoStorage {
    pub async fn new() -> Result<Self, StorageError> {
        let uri = env::var("MONGODB_URI").map_err(|_| {
            StorageError::ConnectionError("MONGODB_URI environment variable not set".to_string())
        })?;

        let client = Client::with_uri_str(&uri).await.map_err(|e| {
            StorageError::ConnectionError(format!("Failed to connect to MongoDB: {}", e))
        })?;
...
impl CloudStorage for MongoStorage {
    async fn upload_repo_data(
        ...
        let mut doc_data = bson::to_document(data).map_err(|e| {
    ...
    async fn upload_type_file(
        ...
        self.collection
            ...
            .await
            .map_err(|e| {
                StorageError::DatabaseError(format!("Failed to upload type file: {}", e))
            })?;
        Ok(())
    }
    ...
    async fn download_all_repo_data(
        ...
        let docs: Vec<bson::Document> = cursor.try_collect().await.map_err(|e| {
            StorageError::DatabaseError(format!("Failed to collect documents: {}", e))
        })?;
        ...
        for doc in docs {
            ...
            if doc.contains_key("endpoints") && doc.contains_key("calls") {
                let repo_data: CloudRepoData = bson::from_document(doc).map_err(|e| {
                    StorageError::SerializationError(format!("Failed to deserialize data: {}", e))
                })?;
