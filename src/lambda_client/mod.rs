...
impl LambdaClient {
    ...
    pub async fn check_or_upload(
        ...
        println!(
            "Calling check-or-upload API for repo: {} ({})",
        ...
        if !response.status().is_success() {
            ...
            return Err(LambdaError::ApiError(format!("HTTP {}: {}", status, body)));
        }
    ...
    pub async fn complete_upload(
        ...
        let request = CompleteUploadRequest {
            ...
            hash: commit_hash.to_string(),
            org: org.to_string(),
            s3Url: s3_url.to_string(),
            filename: "types.tar.gz".to_string(),
        };

        println!(
            "Calling complete-upload API for repo: {} ({})",
            repo_name, commit_hash
        );
    ...
    pub async fn download_adjacent_repo_types(
        ...
        if !output_path.exists() {
            std::fs::create_dir_all(output_path).map_err(|e| {
                LambdaError::S3Error(format!("Failed to create output directory: {}", e))
            })?;
        }

        for adjacent_repo in adjacent {
            println!(
                "Downloading types for adjacent repo: {}",
                adjacent_repo.repo
            );

            // Download tarball from S3
            let response = self.client.get(&adjacent_repo.s3_url).send().await?;

            if !response.status().is_success() {
                println!(
                    "Warning: Failed to download types for repo {}: HTTP {}",
                    adjacent_repo.repo,
                    response.status()
                );
                continue;
            ...
            if !extract_result.status.success() {
                ...
                println!(
                    "Warning: Failed to extract types for repo {}: {}",
            ...
            println!(
                "Successfully downloaded and extracted types for repo: {}",
                adjacent_repo.repo
            );
        }
