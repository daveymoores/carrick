use crate::{
    analyzer::Analyzer,
    cloud_storage::{CloudRepoData, CloudStorage, get_current_commit_hash},
    config::Config,
...
pub async fn run_ci_mode<T: CloudStorage>(storage: T, repo_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    let carrick_token = env::var("CARRICK_TOKEN")
        .map_err(|_| "CARRICK_TOKEN must be set in CI mode")?;
    ...
    storage.upload_repo_data(&carrick_token, &current_repo_data).await
    ...
    let all_repo_data = storage.download_all_repo_data(&carrick_token).await
...
fn analyze_current_repo(repo_path: &str) -> Result<CloudRepoData, Box<dyn std::error::Error>> {
    ...
    println!("---> Analyzing JavaScript/TypeScript files in: {}", repo_path);
    ...
    println!("Found {} files to analyze in directory {}", files.len(), repo_path);
...
fn extract_types_for_current_repo(
    ...
    if !type_infos.is_empty() {
        println!("Processing {} types from repository: {}", type_infos.len(), repo_path);
...
fn build_cross_repo_analyzer(all_repo_data: Vec<CloudRepoData>) -> Result<Analyzer, Box<dyn std::error::Error>> {
    // Combine all configs
    ...
    for repo_data in &all_repo_data {
        ...
        analyzer.imported_handlers.extend(repo_data.imported_handlers.clone());
        analyzer.function_definitions.extend(repo_data.function_definitions.clone());
    }
...
fn print_results(result: crate::analyzer::ApiAnalysisResult) {
    ...
    println!("=====================");
    println!("Found {} endpoints across all files", result.endpoints.len());
