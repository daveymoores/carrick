                ...
                "pubsub_operations": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "topic": {
                                "type": "STRING",
                                "description": "The named topic, channel, or subject the message flows over, as a literal string (e.g., 'metrics.page_view'). This is the first string argument to a publish/subscribe call or the `topic` property in its options object. For a web-platform postMessage envelope, it is instead the string literal on the message object's `action` or `type` property (`parent.postMessage({ action: 'doc-ready', ... }, '*')` → topic 'doc-ready'); on the receiving side, each string-literal dispatch case over that property inside a 'message'-event handler (`switch (event.data.action) { case 'doc-ready': ... }`) is one operation with that topic. Copy it verbatim from the source; skip operations whose topic is a runtime variable with no literal value, and skip postMessage calls whose message has no literal `action`/`type` property (raw data transfers are not pub/sub). The DOM event name 'message' is never itself a topic: a catch-all 'message' listener whose handler does not dispatch on literal envelope-property values yields NO pubsub operations."
                            },
                            "role": {
                                "type": "STRING",
                                "enum": ["subscriber", "publisher"],
                                "description": "A pub/sub operation moves a payload over a named topic/channel/subject. Use `publisher` when the code SENDS a payload to a named topic — for example `producer.send({ topic, messages: [{ value }] })`, `redis.publish(channel, payload)`, or `nc.publish(subject, data)`. Use `subscriber` when the code REGISTERS a handler for (or iterates messages on) a named topic — for example `consumer.subscribe({ topic })` paired with `consumer.run({ eachMessage })`, `sub.subscribe(channel)` paired with `sub.on('message', ...)`, or `nc.subscribe(subject)` consumed with `for await`. These three libraries are only examples of the shape; recognize the same send-to-topic / handle-from-topic shape for any messaging client, including the web-platform postMessage channel: a `postMessage` call carrying a topic-keyed envelope is `publisher`, and a 'message'-event handler dispatching on the envelope's `action`/`type` property is `subscriber` (one operation per dispatch case, even when the handler is registered as a named function reference). Ignore non-pub/sub calls on the same libraries (a key-value `get`/`set`, an admin `createTopics`, or a `request`/reply round-trip are not publish or subscribe)."
                            },
                            "line_number": {
                                "type": "INTEGER",
